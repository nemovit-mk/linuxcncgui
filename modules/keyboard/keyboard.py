...
# ==========================================================
    ...
    def connect_signals(self):
        try:        
            self.widgets.kb11.connect('clicked', self.on_kb_clicked, "A", "7")
            self.widgets.kb12.connect('clicked', self.on_kb_clicked, "B", "8")
            self.widgets.kb13.connect('clicked', self.on_kb_clicked, "C", "9")
            self.widgets.kb14.connect('clicked', self.on_kb_clicked, "D", "/")
            self.widgets.kb15.connect('clicked', self.on_kb_clicked, "E", "(")
            self.widgets.kb21.connect('clicked', self.on_kb_clicked, "F", "4")
            self.widgets.kb22.connect('clicked', self.on_kb_clicked, "G", "5")
            self.widgets.kb23.connect('clicked', self.on_kb_clicked, "H", "6")
            self.widgets.kb24.connect('clicked', self.on_kb_clicked, "I", "*")
            self.widgets.kb25.connect('clicked', self.on_kb_clicked, "J", ")")
            self.widgets.kb31.connect('clicked', self.on_kb_clicked, "K", "1")
            self.widgets.kb32.connect('clicked', self.on_kb_clicked, "L", "2")
            self.widgets.kb33.connect('clicked', self.on_kb_clicked, "M", "3")
            self.widgets.kb34.connect('clicked', self.on_kb_clicked, "N", "-")
            self.widgets.kb35.connect('clicked', self.on_kb_clicked, "O", "[")
            self.widgets.kb41.connect('clicked', self.on_kb_clicked, "P", "=")
            self.widgets.kb42.connect('clicked', self.on_kb_clicked, "Q", "0")
            self.widgets.kb43.connect('clicked', self.on_kb_clicked, "R", ".")
            self.widgets.kb44.connect('clicked', self.on_kb_clicked, "S", "+")
            self.widgets.kb45.connect('clicked', self.on_kb_clicked, "T", "]")
            self.widgets.kb51.connect('clicked', self.on_kb_clicked, "U", "\\")
            self.widgets.kb52.connect('clicked', self.on_kb_clicked, "V", ",")
            self.widgets.kb53.connect('clicked', self.on_kb_clicked, "W", "CHANNEL")
            self.widgets.kb54.connect('clicked', self.on_kb_clicked, "X", "ALARM_CANCEL")
            self.widgets.kb55.connect('clicked', self.on_kb_clicked, "Y", "HELP")
            self.widgets.kb61.connect('clicked', self.on_kb_clicked, "Z", ";")
            self.widgets.kb62.connect('clicked', self.on_kb_clicked, "?", "NEXT_WIN")
            self.widgets.kb63.connect('clicked', self.on_kb_clicked, "!", "UP")
            self.widgets.kb64.connect('clicked', self.on_kb_clicked, "\'", "PAGE_UP")
            self.widgets.kb65.connect('clicked', self.on_kb_clicked, "BACKSPACE", "BACKSPACE")
            self.widgets.kb71.connect('clicked', self.on_kb_clicked, "_", " ")
            self.widgets.kb72.connect('clicked', self.on_kb_clicked, "<", "LEFT")
            self.widgets.kb73.connect('clicked', self.on_kb_clicked, ">", "SELECT")
            self.widgets.kb74.connect('clicked', self.on_kb_clicked, "\"", "RIGHT")
            self.widgets.kb75.connect('clicked', self.on_kb_clicked, "INSERT", "INSERT")
            self.widgets.kb81.connect('clicked', self.on_shift_press)
            self.widgets.kb82.connect('clicked', self.on_kb_clicked, ":", "END")
            self.widgets.kb83.connect('clicked', self.on_kb_clicked, "$", "DOWN")
            self.widgets.kb84.connect('clicked', self.on_kb_clicked, "%", "PAGE_DOWN")
            self.widgets.kb85.connect('clicked', self.on_kb_clicked, "INPUT", "INPUT")
...
# On key pressed
# ==========================================================
    def on_shift_press(self, widget):
        if self.SHIFTED:
            self.SHIFTED = False
        else: self.SHIFTED = True

    def on_kb_clicked(self, widget, *data):
#        if data == None: return
        key_shift, key_unshift = data
        if self.SHIFTED:
            self.place_sign(key_shift)
        else: self.place_sign(key_unshift)
