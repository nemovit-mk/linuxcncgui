...
#        self.widgets.window2.window.connect('destroy', self.on_window_delete_event)
        ...
        self.data.CURRENTSCREEN = "screen_tool"
        self.set_screen()
        self.highliteBTN()
...
#                print("OK")
    ...
    def on_btnX_clicked(self, widget, data=None):
        screen = self.widgets[self.data.CURRENTSCREEN].labelX[data][1]
...
#        screen = self.data.CURRENTSCREEN = self.widgets[self.data.CURRENTSCREEN].labelX[1][1]
#        if screen != "": self.data.CURRENTSCREEN = screen
#        self.set_screen()
